...
def editar_dados():
    ...
    telaeditar.lineEdit_6.setText(str(chamado[0][6]))
...
def salvardadoseditados():
        ...
        solicitante = telaeditar.lineEdit_6.text()
        defeito = telaeditar.elineEdit_7.text()
        atividade = telaeditar.elineEdit_8.text()
        cursor = conn.cursor()
        cursor.execute("UPDATE tabela_relatorio SET tipo_equipamento = '{}', marca = '{}', modelo = '{}', patrimonio = '{}', chamado = '{}', solicitante = '{}', defeito = '{}', atividade = '{}' WHERE id = {}".format(tipo_equipamento, marca, modelo, patrimonio, chamado, solicitante, defeito, atividade, numero_id))
        telaeditar.close()
        segundatela.close()
        chama_segunda_tela()
...
def enviarsaida():
    print("Registor de Saída Enviado com Sucesso!")
    ...
    cursor = conn.cursor()
    conn.execute('''
    INSERT INTO R2 (saida,solucao,tipo_equipamento,marca, modelo, chamado, solicitante, patrimonio, atividade, defeito, entrada)
    VALUES (?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''',
    (data_selecionadas,Linha8,combobox1,Linha1, Linha2, Linha3, Linha4, Linha5, Linha6, Linha7, data_selecionadae))
...
def editarsaida(): 
    ...
    telaeditar2.elineEdit_2.setText(str(chamado[0][2]))
    ...
    telaeditar2.lineEdit_6.setText(str(chamado[0][6]))
    ...
    telaeditar2.elineEdit_11.setText(str(chamado[0][11]))
    


def salvardadoseditadoss():
        global numero_id
        tipo_equipamento = telaeditar2.elineEdit.text()
        marca = telaeditar2.elineEdit_2.text()
        modelo = telaeditar2.elineEdit_3.text()
        patrimonio = telaeditar2.elineEdit_4.text()
        chamado = telaeditar2.elineEdit_5.text()
        solicitante = telaeditar2.lineEdit_6.text()
        defeito = telaeditar2.elineEdit_7.text()
        atividade = telaeditar2.elineEdit_8.text()
        
        cursor = conn.cursor()
        cursor.execute("UPDATE R2 SET saida = '{}', solucao =' {}', tipo_equipamento = '{}', marca = '{}', modelo = '{}', chamado = '{}', solicitante = '{}', patrimonio = '{}', defeito = '{}', atividade = '{}', entrada '{}' '{}' WHERE id = {}".format(tipo_equipamento, marca, modelo, patrimonio, chamado, solicitante, defeito, atividade, numero_id))
        telaeditar2.close() 
        terceiratela.close()
        chama_terceira_tela()
...
def gerar_pdf():
    ...
    estilo_cabecalho = ParagraphStyle(
        name='CustomCabecalhoStyle',
        fontName='Helvetica-Bold',
        fontSize=10,
        leading=12,  # Espaçamento entre linhas
        textColor=colors.whitesmoke  # Cor do texto branco
    )

    # Definindo o estilo do corpo da tabela
    estilo_corpo = ParagraphStyle(
        name='CustomCorpoStyle',
        fontName='Helvetica',
        fontSize=10,
        leading=12,  # Espaçamento entre linhas
    )
    ...
    if dados_lidos:
        ...
        table_large_data = []
        for i, row in enumerate(data_large):
            table_large_row = []
            for j, cell_content in enumerate(row):
                if isinstance(cell_content, str):
                    # Aplica o estilo de cabeçalho apenas para a primeira linha
                    if i == 0:
                        paragrafo = Paragraph(cell_content, estilo_cabecalho)
                    else:
                        paragrafo = Paragraph(cell_content, estilo_corpo)
